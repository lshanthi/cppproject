...
import in.co.mobile.store.from.BookProductForm;
import in.co.mobile.store.from.ProductForm;
import in.co.mobile.store.service.CategoryServiceInt;
import in.co.mobile.store.service.ProductServiceInt;
import in.co.mobile.store.util.DataUtility;
	...
	public String submit(@RequestParam("image") MultipartFile file,@Valid @ModelAttribute("form")  ProductForm form, BindingResult bindingResult,
		...
		try {
			if (OP_SAVE.equalsIgnoreCase(form.getOperation())) {
				...
				fos.close();
				Compress.compress(convFile);
				System.out.println("Before Compress: "+Compress.normal +" After Compress: "+Compress.compress);
				ProductEntity bean = (ProductEntity) form.getBean();
				bean.setImage(file.getBytes());
				if(bean.getId()>0) {
					service.update(bean);
					model.addAttribute("success", "Product update Successfully!!!!"+" Before Compress: "+Compress.normal +" After Compress: "+Compress.compress);
				}else {
				service.add(bean);
				model.addAttribute("success", "Product Added Successfully!!!!"+" Before Compress: "+Compress.normal +" After Compress: "+Compress.compress);
				}
				return "product-form";
			}
