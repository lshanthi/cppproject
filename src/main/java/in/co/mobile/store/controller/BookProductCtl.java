...
import in.co.mobile.store.service.BookProductServiceInt;
import in.co.mobile.store.service.CartServiceInt;
...
public class BookProductCtl extends BaseCtl {
	...
	public String searchList(@ModelAttribute("form") BookProductForm form,
			@RequestParam(required = false) String operation,Long bid,HttpSession session, Model model) {
		...
		if (OP_DELETE.equals(operation)) {
			pageNo = 1;
			BookProductEntity bPro=service.findByPk(bid);
			service.delete(bPro);
			return "redirect:/bookProd/search";
		}
