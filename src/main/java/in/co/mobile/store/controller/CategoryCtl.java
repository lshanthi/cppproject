	...
	public String submit(@RequestParam("image") MultipartFile file,@Valid @ModelAttribute("form")  CategoryForm form, BindingResult bindingResult,
		...
		try {
			if (OP_SAVE.equalsIgnoreCase(form.getOperation())) {
				...
				if(bean.getId()>0) {
					service.update(bean);
					model.addAttribute("success", "Category update Successfully!!!!"+" Before Compress: "+Compress.normal +" After Compress: "+Compress.compress);
				}else {
				service.add(bean);
				model.addAttribute("success", "Category Added Successfully!!!!"+" Before Compress: "+Compress.normal +" After Compress: "+Compress.compress);
				}
				return "category-form";
			}
