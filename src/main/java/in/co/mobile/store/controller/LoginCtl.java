...
import in.co.mobile.store.from.LoginForm;
import in.co.mobile.store.from.UserRegistrationForm;
import in.co.mobile.store.service.UserServiceInt;
...
/*
 * Display method parameters pid, form, session and model
 * Get http method is /login  
 */
	@GetMapping("/login")
	public String display(@RequestParam(required = false) Long pid,@ModelAttribute("form") LoginForm form, HttpSession session, Model model) {
		...
		// Setting pid into the session variable.
